Sub AddVLookupFormulaIfMedicalClaim()
    Dim ws As Worksheet
    Dim lastRow As Long
    Dim i As Long
    ...
    For i = 2 To lastRow ' Start from row 2 to skip header
        ' Check if column A contains "Medical Claim"
        If ws.Cells(i, 1).Value = "Medical Claim" Then
            ' Set the VLOOKUP formula in column E for the matching row
            formula = "=VLOOKUP(B" & i & ", '[Template SAP.xlsx]MEDICAL'!$Y$2:$Y$1000000, 1, FALSE)"
